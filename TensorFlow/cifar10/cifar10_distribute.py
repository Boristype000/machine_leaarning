...
"""
this file is used to run cifar10 on meituan-yun
in distribution way
"""
...
def train():
    ...
    if FLAGS.job_name == "ps":
        ...
        task_index = "/job:worker/task:%d" % FLAGS.task_index
        with tf.device(tf.train.replica_device_setter(worker_device=task_index,
                                                      cluster=cluster)):
            ...
            with tf.variable_scope(tf.get_variable_scope()):
                for i in xrange(FLAGS.num_gpus):
                    gpu_index = "{0}/gpu:{1}".format(task_index,i)
                    with tf.device(gpu_index):
                        # with tf.device('/gpu:%d' % i):
                        with tf.name_scope('%s_%s_%d' % (FLAGS.task_index, cifar10.TOWER_NAME, i)) as scope:
